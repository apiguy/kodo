...
RSpec.describe Kodo::Tools::RememberFact, :tmpdir do
  ...
  describe 'web content memory poisoning gate' do
    let(:turn_context) { Kodo::Web::TurnContext.new }

    before { tool.turn_context = turn_context }

    it 'allows remember when no web content was fetched' do
      result = tool.execute(category: 'fact', content: 'Likes Ruby')
      expect(result).to include('Remembered')
    end

    it 'blocks remember when web_fetched flag is set' do
      turn_context.web_fetched!

      result = tool.execute(category: 'fact', content: 'Likes Python')
      expect(result).to include('Web content was fetched this turn')
      expect(result).to include('memory poisoning')
      expect(knowledge.count).to eq(0)
    end

    it 'echoes back the content in the confirmation gate message' do
      turn_context.web_fetched!

      result = tool.execute(category: 'fact', content: 'Important fact')
      expect(result).to include('Important fact')
    end

    it 'works without a turn_context (nil safety)' do
      tool.turn_context = nil

      result = tool.execute(category: 'fact', content: 'Should work')
      expect(result).to include('Remembered')
    end
  end
