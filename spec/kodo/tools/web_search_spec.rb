...
RSpec.describe Kodo::Tools::WebSearch, :tmpdir do
  ...
  let(:tool) do
    t = described_class.new(search_provider: search_provider, audit: audit)
  ...
  describe 'nonce wrapping and turn_context' do
    it 'wraps results in nonce markers' do
      allow(search_provider).to receive(:search).and_return(sample_results)
    ...
    it 'sets web_fetched! on turn_context after search' do
      allow(search_provider).to receive(:search).and_return(sample_results)

      expect { tool.execute(query: 'Ruby 3.3') }
        .to change { turn_context.web_fetched }.from(false).to(true)
    end

    it 'sets web_fetched! even when no results found' do
      allow(search_provider).to receive(:search).and_return([])

      expect { tool.execute(query: 'nothing') }
        .to change { turn_context.web_fetched }.from(false).to(true)
    end
  end
